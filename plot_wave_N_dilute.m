...
% evaluate u_sc and u_in on the domain
for i = 1:N
    ...
    if i == 1 
        ...
        u_in = @(x,t) get_u_from_v(x,t,v_in,k_tr,w_op,Omega); % left incident wave field of D_{i}
    end
...
for i = (N-1):(-1):1
    ...
    % define the scattered wave field
    v_sc = @(x,n) all_betas_r(i,n+k_tr+1)*exp(-sqrt(-1)*(x)*(w_op+n*Omega)/v0).*(x<=z(i))+all_alphas_r(i,n+k_tr+1)*exp(sqrt(-1)*(x)*(w_op+n*Omega)/v0).*(x>z(i));
    u_sc = @(x,t) get_u_from_v(x,t,v_sc,k_tr,w_op,Omega);

    % define the incident wave to D_i
    v_in = @(x,n) all_betas_l(i+1,n+k_tr+1)*exp(-sqrt(-1)*(x)*(w_op+n*Omega)/v0).*(x>z(i));
    u_in = @(x,t) get_u_from_v(x,t,v_in,k_tr,w_op,Omega); % right incident wave field of D_{i}

    for k = 1:len_xs
        x = all_xs(k);
        all_uxs_r(k) = all_uxs_r(k) + u_sc(x,t);
        all_uxs_in_r(k) = all_uxs_in_r(k) + u_in(x,t);
    end
