...
for i = 1:N
    ...
    % left incident wave
    if i == 1
        uin_l = @(x,t,n) exp(sqrt(-1)*((k0).*x+w0.*t)).*(x<xm(i)).*(n==0); % left incident wave
...
all_uxs_l = zeros(1,(N+1)*len_xs); all_uxs_r = zeros(1,(N+1)*len_xs);
all_uxs_in_l = zeros(1,(N+1)*len_xs); all_uxs_in_r = zeros(1,(N+1)*len_xs);
t = 0;
...
for i = 1:(N+1)
    ...
    if i == 1
        ...
        u_in = @(x,t) exp(sqrt(-1)*((k0).*x+w0.*t)); % left incident wave of D_1
    elseif i < N+1
        ...
        v_in = @(x,n) all_alphas_l(N,n+k_tr+1)*exp(sqrt(-1)*x*(w_op+n*Omega)/v0);
        u_in = @(x,t) get_u_from_v(x,t,v_in,k_tr,w_op,Omega); % left incident wave field of D_N
    end

    for k = 1:len_xs
        x = all_xs((i-1)*len_xs+k);
        all_uxs_l((i-1)*len_xs+k) = u_sc(x,t);
        all_uxs_in_l((i-1)*len_xs+k) = u_in(x,t);
    end
