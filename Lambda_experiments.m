...
for len = all_len
    ...
    w = w0 + 0.000001; % quasifrequency of incident wave
...
for n = 1:(2*k_tr+1)
    subplot(1,2,1)
    plot(all_len,real(all_Ln(n,:)),'-','Color',c_map(iR,:),'DisplayName',strcat('$n=$',num2str(n-(k_tr+1))),markersize=8,linewidth=2)
    hold on
    subplot(1,2,2)
    plot(all_len,imag(all_Ln(n,:)),'-','Color',c_map(iR,:),'DisplayName',strcat('$n=$',num2str(n-(k_tr+1))),markersize=8,linewidth=2)
    hold on    
    iR = iR+2;
end
% add legends etc.
...
ylabel('Re$(\Lambda_n(\ell))$',fontsize=18,Interpreter='latex')
subplot(1,2,2)
legend('show',interpreter='latex',fontsize=18,location='southoutside')
xlabel('$\ell$',fontsize=18,Interpreter='latex')
ylabel('Im$(\Lambda_n(\ell))$',fontsize=18,Interpreter='latex')
...
N = 1; % number of the resonator
lij = ones(1,N-1); % spacing between the resonators
len = 10; li = ones(1,N).*len; % length of the resonator
xm = 0; xp = xm+li(1);
delta = 0.0001; % small contrast parameter
...
for epsilon_kappa = all_epsk % iterate over modulation amplitudes of kappa
    ...
    all_Ln(:,c) = get_Lambdas(xm, xp, k_tr, w, Omega, rs, ks, vr, delta, v0, k, k_0)'; % \Lambda_n for all n=-K,...,K
    c = c+1;
