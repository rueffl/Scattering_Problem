...
% lij = ones(1,N-1).*50; %lij(2:2:end) = lij(2:2:end)+1; % distance between the resonators
...
%     us = 0;
%     for n = -k_tr:k_tr
%         us = us + all_cn(n+k_tr+1).*G(x,(w+n*Omega)./v0).*exp(sqrt(-1)*(w0+n*Omega).*t);
%     end
%    us_x(j) = us;
%    j = j + 1;
% end
...
% [us, xs] = prepare_plot(k_tr,li,lij,L,N,Omega,delta,phase_kappa,phase_rho,epsilon_kappa,epsilon_rho,xm,xp,vr,v0);%% N Resonators
% Settings for the structure
